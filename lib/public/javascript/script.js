...
//*********************TEST FACTO SOCKET SIZE ****************//
//function socket_size_map (name_emit, canvas_size, canvasWH) {
//    socket.emit(name_emit, canvas_size);
//    return socket.on(name_emit, function(name_emit) {
//        console.log(name_emit);
//        canvasWH = name_emit;
//    });
//}
//socket_size_map('canvas_front_width', canvasWidth.value, canvasFront.width);
//socket_size_map('canvas_back_width', canvasWidth.value, canvasBack.width);
//socket_size_map('canvas_front_height', canvasHeight.value, canvasFront.height);
//socket_size_map('canvas_back_height', canvasHeight.value, canvasBack.height);
//*********************END FACTO SOCKET SIZE *****************//
...
function drag(event) {
    ...
    var position = getCanvasCoordinates(event);
        ...
        if(status_rect) {
            drawRect(position);
            socket.emit('ctxbb_drag', ctxb);
            socket.on('ctxbb_drag', function(ctxb) {
                console.log(ctxb);
            });
            socket.emit('ctxff_drag', ctxf);
            socket.on('ctxff_drag', function(ctxf) {
                console.log(ctxf);
            });
            //var socket_rect = drawRect(position);
        }
        if(status_rect_v) {
           drawRect(position);
        }
    ...
    socket.emit('drawPolygon_v_drag', drawPolygon(position, 6, Math.PI / 4));
        }
        if(status_circlee_v) {
           drawCircle(position);
    socket.emit('drawCircle_v_drag', drawCircle(position));
        }
        if(status_circlee) {
            drawCircle(position);
    socket.emit('drawCircle_drag', drawCircle(position));  
        }
        if(status_polygon) {
            drawPolygon(position, 6, Math.PI / 4);
    socket.emit('drawPolygon_drag', drawPolygon(position, 6, Math.PI / 4));
        }
    }
}
function dragStop(event) {
    ...
    if(!status_eraser) {
        ...
        if(status_rect) {
            drawRect(position);
            socket.emit('ctxbb_stop', ctxb);
            socket.on('ctxbb_stop', function(ctxb) {
                console.log(ctxb);
            });
            socket.emit('ctxff_stop', ctxf);
        ...
        if(status_circlee) {
            drawCircle(position);
    socket.emit('drawCircle_dragStop', drawCircle(position));
        }
        if(status_polygon) {
            drawPolygon(position, 6, Math.PI / 4);
        }
